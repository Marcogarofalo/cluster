...
int main(int argc, char** argv) {
  ...
  double *lex_phi;
    ...
    lex_phi=(double*) malloc(sizeof(double)*4*L[0]*L[1]*L[2]*L[3]);
...
for (ii=params.data.start_measure; ii<params.data.total_measure; ii+=params.data.save_config_every_X_updates){  
  ...
  std::string conf_file = "./data/T" + std::to_string(params.data.L[0]) +
      ...
      mdp << "Reading configuration in: " << conf_file << endl;
  std::string conf_out = "./scalars_lex/T" + std::to_string(params.data.L[0]) +
                              ".X" + std::to_string(params.data.L[1]) +
                              ".Y" + std::to_string(params.data.L[2]) +
                              ".Z" + std::to_string(params.data.L[3]) +
                              ".kap" + std::to_string(params.data.kappa) +
                              ...
                              ".rot_" + params.data.save_config_rotated +
                              ".conf" + std::to_string(ii);
      mdp << "opening configuration in: " << conf_out << endl;

  FILE *f_in,*f_out;

  f_in=fopen(conf_file.c_str(),"r");

  if (f_in != NULL) {

    f_out=fopen(conf_out.c_str(),"w+");
    if (f_out == NULL) {
    ...
    printf("reading\n");
    forallsites(x){
        fread( &phi(x),4*sizeof(double),1,f_in );
       
        
    }
    
    printf("converting 2 lex\n");
    int  i, x1,x2,x3,lex;
    for(i=0; i<L[0]; i++) {
        for(x1=0; x1<L[1]; x1++){
            for(x2=0; x2<L[2]; x2++){
            for(x3=0; x3<L[3]; x3++) {
                x.set(i,x1,x2,x3);
                lex=x3+x2*L[1]+x1*L[1]*L[2]+ i*L[1]*L[2]*L[3];
                if (lex !=x.global_index()) {
    ...
    printf("writing\n");
    fwrite( lex_phi ,sizeof(double),4*L[0]*L[1]*L[2]*L[3],f_out  );
