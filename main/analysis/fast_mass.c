...
int *w,N_tot,every,call_f,global_k;
...
//   for(i1=0;i1<=80;i1++)
//   printf("%f    %f   %f\n",gammaFbb[0][0],tmp[i1][0],fbba[i1][0]);
...
/*if(i==0) printf("%f\n",tmp[i][0][j]+0.837423938138333);*/
    ...
    for ( k=0; k< Nfit; k++ ){
        ...
        for ( i = 0; i < L0; i++){
            mean_value(var,order,replicas,N_tot/NxG,i+L0,tmp1);
            windowing(var,order,replicas,N_tot/NxG,i+L0,tmp1,abb);
   ...
   /* 
    ...
    for ( k=0; k< Nfit; k++ ){
        ...
        fit=constant_fit(9,18,effective_mass[k]);
        fprintf(file_out,"\n\n");
