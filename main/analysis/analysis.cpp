...
int main (int argc, char** argv) {
  int verbose = 0;
  int order=0,alpha=1,conf,var=1,replicas=1,j,i,k,l,NxG0=8, therm=0;
  double ***tmp,mean=0,sigma=0,**tmp1;
	...
	for(k=0;k<2;k++){  
        	tmp[i][k]=(double*) malloc(N_tot*sizeof(double));
	}
    }
 
    tmp1=(double**) malloc(sizeof(double*)*2 );
    tmp1[0]=(double*) calloc(L0,sizeof(double) );
    tmp1[1]=(double*) calloc(L0,sizeof(double) );
...
double a,b,c,d;
    for(j=0;j<N_tot;j++){
			...
			if (time!=i){
				printf("error in reading the time coordinate sum orver scalar\n read=%d expected=%d\n",time,i); exit(0);
			}
		}
    }
    for(j=0;j<N_tot;j++)
    ...
    for(l=0;l<4;l++)
    	tmp1[1][l]/=(double) N_tot;
 
    fclose(file_in);
    FILE  *file_out=NULL;
    file_out=fopen(output_filename,"ab");
    if(file_out==NULL){
	printf("unable to open output file"); exit(0);
    }

    FILE  *file_out1=NULL;
    file_out1=fopen("tmp5","ab");
    if(file_out1==NULL){
	printf("unable to open output file"); exit(0);
    }
    fprintf(file_out1,"%.15g\t %.15g\t %.15g\t %.15g\t 10\n ",tmp1[1][1],tmp1[1][2],tmp1[1][3],tmp1[1][0]);
    for(i=0;i<L0;i++)    
    	fprintf(file_out,"%d \t  %.15g \t \n",i,tmp1[0][i]);
 /* 
   var=2*L0; 
   mean_value(var,order,replicas,N_tot/NxG,0,tmp1);
   for(i=0;i=T/2;i++){
